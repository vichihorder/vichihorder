...
$orders = \Illuminate\Support\Facades\DB::select("
            select * from `order` where `status` = '".App\Order::STATUS_TRANSPORTING."' {$and_where} and `transporting_at` <= '".$l_time."'
        ");
if(count($orders)){
?>
...
<table class="table">
    ...
    <tr>
        ...
        <td>
            ...
            <small>(<abbr title="{{$order->transporting_at}}" class="_time-ago"></abbr>)</small>
        </td>
        <td>
