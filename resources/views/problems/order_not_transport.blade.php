...
<?php
//Đơn quá ? ngày mà chưa vận chuyển về việt nam, kể từ khi kho trung quốc nhận
...
$orders = \Illuminate\Support\Facades\DB::select("
            select * from `order` where `status` = '".App\Order::STATUS_RECEIVED_FROM_SELLER."' {$and_where} and `received_from_seller_at` <= '".$l_time."'
        ");
if(count($orders)){
?>
...
<table class="table">
    ...
    <tr>
        ...
        <td>
            {{App\Util::formatDate($order->received_from_seller_at)}}
            <small>(<abbr title="{{$order->received_from_seller_at}}" class="_time-ago"></abbr>)</small>
        </td>
        <td>
